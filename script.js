...
// Local save only to prevent circular calls during initial load
...
function loadState() {
    ...
    if (state.courses.length === 1 && state.courses[0].title === "Example Course" && state.courses[0].modules.length === 0) {
        state.courses = DEFAULT_COURSES.map(title => ({
            id: generateId(),
            title: title,
            isCollapsed: true,
    ...
    // We delay the cloud check slightly to ensure firebase module attaches to window
    setTimeout(() => {
        if (window.cloudLoadState) {
            const globalProgText = document.getElementById('global-progress-text');
            if (globalProgText) globalProgText.innerHTML = '<span class="material-symbols-rounded spin-anim" style="font-size:16px;">sync</span>';
            
            window.cloudLoadState().then(cloudState => {
                if (cloudState) {
                    state = cloudState;
                    saveStateLocalOnly();
                }
                ...
                // Keep the UI toggle synced with state
                if (state.isEditMode) {
...
function exportData() {
    ...
    downloadAnchorNode.setAttribute("download", "note_progress_backup.json");
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (importedState && importedState.courses) {
                state = importedState;
                saveState();
                render();
